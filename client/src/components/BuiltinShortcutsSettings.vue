  ...
  >
    <div class="builtin-dialog">
      <label class="builtin-dialog__delay">
        ...
        <InputNumber v-model="startDelaySeconds" :min="0" :max="60" :step="5" showButtons buttonLayout="horizontal" :inputStyle="{ width: '3.5rem' }" />
      </label>

      <label class="builtin-dialog__delay">
        <span>Timer duration (seconds)</span>
        <InputNumber v-model="timerSeconds" :min="5" :max="600" :step="15" showButtons buttonLayout="horizontal" :inputStyle="{ width: '3.5rem' }" />
      </label>
      ...
      <div class="builtin-dialog__list">
        <div v-for="row in rows" :key="row.id" class="builtin-dialog__row">
          ...
          <div class="builtin-dialog__toggle">
            <ToggleSwitch v-model="builtinWarnings[row.id]" class="app-solid-switch" />
